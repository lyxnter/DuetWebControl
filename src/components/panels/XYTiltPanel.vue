...
<template>
  <v-expansion-panel :value="isLocal?-1:0" class="card x_y_tilt">
    <v-expansion-panel-content>
      <template v-slot:header>
        <div>	{{ $t('panel.tiltCompensation.caption') }} </div>
      </template>
      <v-card>
        <v-card-text class="panel-body" style="width: max-content; border-right: 1px solid #333; border-bottom: 1px solid #222; margin: 5px auto; text-align: center; background: #505050;">
          <template v-if="asDeg">
            <p><span v-html="$t('panel.tiltCompensation.tilt', ['A'])"></span> :
              ...
              <span style="color: darkgray">{{ $t('panel.tiltCompensation.rotation', ['X']) }}</span>
            </p>
            <v-tooltip bottom>
              ...
              <span> {{ $t('panel.tiltCompensation.buttonCaption.decrease', ['X', 'B']) }} </span>
            </v-tooltip>
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
              ...
              <span> {{ $t('panel.tiltCompensation.buttonCaption.increase', ['X', 'B']) }} </span>
            </v-tooltip>
            <br/>
            <p> <span v-html="$t('panel.tiltCompensation.tilt', ['B'])"></span> :
              <input style="width: 50px; height: 20px;text-align: center; border: 1px solid black; border-radius: 4px;"
              axis="b"
              class="span_tilt"
              autocomplete="off"
              ...
              @keypress="keypressTiltEvent"
              @blur="blurTiltEvent"/> &deg;<br/>
              <span style="color: darkgray">{{ $t('panel.tiltCompensation.rotation', ['Y']) }}</span>
            </p>
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
              ...
              <span> {{ $t('panel.tiltCompensation.buttonCaption.decrease', ['Y', 'A']) }} </span>
            </v-tooltip>
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
              ...
              <span> {{ $t('panel.tiltCompensation.buttonCaption.increase', ['Y', 'A']) }} </span>
            </v-tooltip>
          </template>
          <template v-if="asPrct">
            <p> <span v-html="$t('panel.tiltCompensation.offset', ['X'])"></span> :
              <input style="width: 50px; height: 20px;text-align: center; border: 1px solid black; border-radius: 4px;"
              class="span_tilt"
              axis="x"
              autocomplete="off"
              ...
              <span style="color: darkgray"><span v-html="$t('panel.tiltCompensation.tooltip', ['X'])"></span></span>
            </p>
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
            ...
            <br/>
            <p> <span v-html="$t('panel.tiltCompensation.offset', ['Y'])"></span> :
              <input style="width: 50px; height: 20px;text-align: center; border: 1px solid black; border-radius: 4px;"
              axis="y"
              class="span_tilt"
              autocomplete="off"
              ...
              @keypress="keypressTiltEvent"
              @blur="blurTiltEvent"/> %<br/>
              <span style="color: darkgray"><span v-html="$t('panel.tiltCompensation.tooltip', ['Y'])"></span></span>
            </p>
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
          ...
          <div style="width: max-content; margin: auto;">
            {{ $t('panel.tiltCompensation.switch') }}<br/>
